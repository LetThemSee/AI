...
class State2048:
    ...
    def calculate_utility(self):
        ...
        # Heuristic 2: reward
        weights = np.array([[11, 9, 7, 5], [9, 7, 5, 3], [7, 5, 3, 1], [5, 3, 1, 0]])
        ...
        # Heuristic 5: penalty
        #penalty += self.calculate_penalty_smallValue()
        ...
        for pos in filled_cells:
            ...
            for n_pos in neighbors:
                if not self.grid.crossBound(n_pos):
                    '''
                    if self.grid.map[pos[0]][pos[1]] != self.grid.map[n_pos[0]][n_pos[1]]:
                        penalty += 1
                    '''
    ...
    def calculate_reward_sameNeighbor(self):
        reward = 0
        filled_cells = self.get_filled_cells()
        for pos in filled_cells:
            # Neighbors: Up Down Left Right
            ...
            val = self.grid.map[pos[0]][pos[1]]
            for n_pos in neighbors:
                if not self.grid.crossBound(n_pos):
                    if val == self.grid.map[n_pos[0]][n_pos[1]]:
                        reward += 1
            
        return reward
    
    def calculate_penalty_smallValue(self):
        penalty = 0
        for i in range(2):
            for j in range(2):
                pos = (i, j)
                val = self.grid.getCellValue(pos)
...
if __name__ == '__main__':
    ...
    state = State2048(g)
    print(state.calculate_reward_sameNeighbor())
    print()
